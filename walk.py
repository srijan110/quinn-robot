...
from body import init_servo, set_leg_servo_angles, set_hip_servo_angles
...
def turn(left=True, ix=1, iy=9, fx=-1, fy=9, hip_offset=-15, ankle_offset=0, step_duration=0.2, delay=0.01, dx=-1, dy=9, steps=1):
    ...
    while step_index != steps:
        ...
        if left:
            left_leg = [degrees(x) for x in get_angles(dx, dy-1, 5, 5, 3)]
            right_leg = [degrees(x) for x in get_angles(rx, ry, 5, 5, 3)]
        else:
            left_leg = [degrees(x) for x in get_angles(lx, ly, 5, 5, 3)]
            right_leg = [degrees(x) for x in get_angles(dx, dy-1, 5, 5, 3)]
...
def walk_front(ix=1, iy=9, fx=-2, fy=9, hip_offset=-15, ankle_offset=0, step_duration=0.2, delay=0.01, dx=-1, dy=9, steps=1):
    left_leg, right_leg = ([degrees(x) for x in get_angles(dx, dy, 5, 5, 3)], [degrees(x) for x in get_angles(-1, 9, 5, 5, 3)])
    left_leg[0] = right_leg[0] = left_leg[0] + hip_offset
    set_leg_servo_angles(left_leg, right_leg)
